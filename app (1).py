...
# ================= CRITICAL FIX: DEFINE ALL SIDEBAR VARIABLES FIRST =================
# Since Python executes top-to-bottom, we must define all sidebar UI elements
# BEFORE rendering any of the tabs. This completely prevents any NameError!
...
with st.sidebar:
    ...
    jewelry = st.selectbox(
        "Traditional Jewelry",
        [
            "Jonbiri (Half-moon gold pendant) and Lokaparo (Pigeon gold earrings)",
            "Dholbiri (Assamese drum pendant) and Gam-kharu (Wide gold bangles)",
            "Kopo Phul (Foxtail orchid) gold earrings and Gal-patra necklace",
            "Modern minimalist gold jewelry set",
            "None (No jewelry)"
        ], index=0, key="global_jew"
    )
    
    background_scene = st.selectbox(
        "Backdrop Setting",
        [
            "Minimalist professional studio setting with soft warm shadows",
            "Lush green Assam tea garden with rolling misty hills",
            "Traditional Assamese rural courtyard with betel nut palms and brass Xorai",
            "Historic Sivasagar Rang Ghar royal pavilion in the background",
            "Modern high-fashion runway with spotlights"
        ], index=0, key="global_bg"
    )
    
    camera_lighting = st.selectbox(
        "Lighting Setup",
        [
            "Professional high-fashion studio strobe lighting",
            "Soft natural golden hour sunlight",
            "Dramatic low-key cinematic side lighting",
            "Bright, vibrant outdoor daylight"
        ], index=0, key="global_light"
    )
